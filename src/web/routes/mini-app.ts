...
import { tmaSkillsPage, tmaSkillDetailPage } from "../views/mini-app/skills.ts";
...
import { counts as atlasCounts, listByKind as atlasListByKind, getItem as getAtlasItem, type AtlasKind } from "../../atlas/index.ts";
...
export function createMiniAppRoutes() {
  ...
  app.get("/library/synthesis", (c) => {
    ...
    let active: SynthesisFile | null = null;
    let content: string | null = null;
    if (reqPath) {
      active = files.find((f) => f.path === reqPath) ?? null;
      if (active) content = safeReadSynthesis(active.path);
    } else if (files.length > 0) {
      active = files[0];
      content = safeReadSynthesis(active.path);
    }
    const health = libraryHealth();
    return c.html(
      tmaSynthesisPage({
